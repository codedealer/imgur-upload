...
:: If arguments provided, use them directly (for command line usage)
if not "%~1"=="" (
    set NODE_ENV=production
    cd dist
    echo Starting reupload process...
    node index.js %*
    pause
    exit /b 0
)

:: Interactive mode - launch app with special flag
set NODE_ENV=production
cd dist
node index.js --interactive-reupload
